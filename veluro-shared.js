...
function getCartTotalWas() { return getCart().reduce((s,i)=>s+(i.was||i.price)*i.qty,0); }

function pushToCart(name, color, price, img, was) {
  const cart = getCart();
  const key = name+'|'+color;
  const ex = cart.find(i=>i.key===key);
  if (ex) ex.qty++; else cart.push({key,name,color,price,was:was||null,qty:1,img});
  saveCart(cart);
  updateAllCartCounts();
}
...
function renderCart() {
  ...
  items.innerHTML=cart.map((it,i)=>{
    const priceNow = (it.price*it.qty).toFixed(2).replace('.',',');
    const priceWas = it.was ? (it.was*it.qty).toFixed(2).replace('.',',') : null;
    return `
    <div class="cart-li">
      ...
      <div class="cart-li-prices">
        <span class="cart-li-price">${priceNow}€</span>
        ${priceWas ? `<span class="cart-li-was">${priceWas}€</span>` : ''}
      </div>
    </div>`;
  }).join('');
  // Total
  const totalNow = getCartTotal().toFixed(2).replace('.',',');
  const totalWas = getCartTotalWas().toFixed(2).replace('.',',');
  const totalEl = document.getElementById('cart-total');
  if(totalEl) totalEl.innerHTML = `${totalNow}€ ${totalWas !== totalNow ? `<span class="cart-total-was">${totalWas}€</span>` : ''}`;
}
